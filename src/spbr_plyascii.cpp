...
SPBR::readPLYHeader_and_countParticles ( void )
{
  ...
  m_numParticles = count_ply_vertex ( m_input_file );//TANAKA 181230 (unchanged)
...
SPBR::readPLYPointData( void )
{
  ...
  unsigned long point_counter = 0 ;//TANAKA 181230
  ...
  while ( reader.getline( buf, SPBR_BUF_SIZE - 1 , '\n' ) ) {
	...
	break; // get out of the while loop to read lines
      ...
      if(m_property == XYZ){
        ...
        // Shading: Use the default setting or
	//          the user-setting of the #/Shading command.
        //setFlagUseNormals(true); //TANAKA 2021/01/17 XXX
    ...
    if( buf[0] == '#' ) {
	  ...
	  double kd, ka, ks;
        ...
        if ( !strncmp( buf, NUM_PARTICLES_COMMAND , strlen(NUM_PARTICLES_COMMAND) ) ) { 
          unsigned long n_tmp; //TANAKA 181230
          sscanf ( buf, "%s %lu", dummy, &n_tmp ); //TANAKA 181230
          m_numParticles = (unsigned long)n_tmp; //TANAKA 181230
	  ...
	  double zoom_speed = 1.0;// initialize to the default value
	   ...
	   sscanf( buf, "%s %lg %d %s %s", dummy, &end_rot_angle, &num_rot_images, rot_axis, loop_image_name_head );
           setEndRotAngle ( end_rot_angle ) ;
           setNumRotImages( num_rot_images) ;
           setRotAxis     ( rot_axis ) ;
           setLoopImageNameHead ( loop_image_name_head );
